...
var lapid = function () {
	...
	!function e(t, n, r) {
		function s(o, u) {
			if (!n[o]) {
				if (!t[o]) {
					var c = "function" == typeof commonjsRequire && commonjsRequire;if (!u && c) return c(o, !0);if (i) return i(o, !0);var a = new Error("Cannot find module '" + o + "'");throw a.code = "MODULE_NOT_FOUND", a;
		...
		119: [function (t, n, r) {
			...
			var e = t(25),
			    ...
			    a = t(118),
			    f = t(24),
			    s = t(129);i(i.S + i.F * !t(56)(function (t) {}), "Array", { from: function from(t) {
					var n,
					    r,
					    i,
					    l,
	...
	var getJSON = function getJSON(url, fullfilled, rejected) {
	...
	var postJSON = function postJSON(url, json, fullfilled, rejected) {
		window.fetch(url, {
			headers: {
	...
	var weightedRandom = function weightedRandom(spec) {
		var table = [];
	...
	var Ngram = function Ngram(options) {
		...
		if (options.index === 0 && options.wordIndex === 0 && options.lineIndex === 0) {
			for (var i = 0; i < store.text.length - options.limit; i++) {
				var ngram = store.text.substring(i, i + options.limit);
			...
			var possibilites = store.markovChain[store.ngramsArray[options.index - 1]];

			if (possibilites) {
				var nextLetter = utils.weightedRandom(possibilites)();

				store.ngrams += nextLetter;

				var currentNgram = store.ngrams.substring(store.ngrams.length - options.limit, store.ngrams.length);
  ...
  */
	var Line = function Line(newOptions, resolve) {
		var options = utils.mergeDeep(DEFAULTS.line, newOptions);
   ...
   */
		var generate = function generate() {
			...
			for (var i = 0; i < options.limit; i++) {
				...
				var next = utils.weightedRandom(possibilites)();
		...
		if (language.corpus) {
			...
			utils.getJSON("../../corpora/de.json", function (corpus) {
				console.time();
				...
				var loop = function loop() {
					...
					setTimeout(function () {
						if (corpusIndex < corpus.length) {
							...
							var ngram = language.corpus[corpusIndex].substring(0, options.word.ngram.limit);
  ...
  */
   ...
   */
    ...
    */
	...
	var Lines = function Lines(newOptions) {
		var _this3 = this;

		return new Promise(function (resolveLines) {
			var options = utils.mergeDeep(DEFAULTS.lines, newOptions);
					...
					while (1) {
						switch (_context4.prev = _context4.next) {
											...
											switch (_context3.prev = _context3.next) {
												case 0:
													...
													return new Promise(function (resolveLine) {
														_this3[i] = new Line(lineOptions, resolveLine);
													}).then(function () {
														_this3.text += _this3[i].text + '\n';
													});
							...
							case 2:
								...
								return _context4.delegateYield(_loop2(i), 't0', 4);
